    ...
    m_mappedVertexArray(vtkSmartPointer<vtkDoubleArray>::New())
{
    ...
    queueConnect<Event>(m_geometry->getIndices(), &VecDataArray<int, 2>::modified, this, &VTKLineMeshRenderDelegate::indexDataModified);
...
VTKLineMeshRenderDelegate::processEvents()
{
    ...
    std::shared_ptr<VecDataArray<int, 2>>    indicesPtr       = m_geometry->getIndices();
...
{
    setIndexBuffer(m_geometry->getIndices());
}
...
VTKLineMeshRenderDelegate::geometryModified(Event* imstkNotUsed(e))
{
    ...
    if (m_indices != m_geometry->getIndices())
    {
        setIndexBuffer(m_geometry->getIndices());
    }
