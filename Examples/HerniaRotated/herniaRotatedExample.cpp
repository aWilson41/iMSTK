...
#include "VRHandControl.h"

//#define USE_VR
//#define USE_ONE_HAPTIC_DEVICE
...
const double simTimestep       = 0.0025; // 0.0025
...
const double lapToolCapsuleLength = 0.5;

// In this example we have:
// - A deformable stomach
// - A diaphgram SDF which helps hold the stomach in place
// - <currently omitted> 3 ligaments attached to the diaphgram which help hold it in place.
// - 3 tools. 2 controlled by devices. These can grasp the stomach to wrap around the esophagus.
// - A string of connected capsule running down the esophagus and side of stomach
...
makeStomachObj(const std::string&        name,
              std::shared_ptr<PbdModel> model)
{
    ...
    for (auto i : fixedMap)
    {
        fixedParticles.push_back(i.second);
    }
    ...
    // Instead of infinite mass fixed points we use springs to infinite mass fixed points
    model->getConfig()->addPbdConstraintFunctor([=](PbdConstraintContainer& container)
        {
            std::shared_ptr<PbdModel> pbdModel = tissueObj->getPbdModel();
            for (auto fixedParticleVertId : fixedParticles)
            {
                const PbdParticleId particleId = { tissueObj->getPbdBody()->bodyHandle, fixedParticleVertId };
                const PbdParticleId& virtualId = pbdModel->addVirtualParticle(pbdModel->getBodies().getPosition(particleId), 0.0, Vec3d::Zero(), true);

                auto distConstraint = std::make_shared<PbdDistanceConstraint>();
                distConstraint->initConstraint(0.0, particleId, virtualId, 10.0);
...
///
/// \brief Creates the capsule running down the esophagus and stomach
/// 
void
...
///
/// \brief Creates pbd rigid laparascopic tool objects
/// 
...
///
/// \brief Create a pbd rigid hand to grasp the tool
///
std::shared_ptr<PbdObject>
            ...
            std::shared_ptr<PbdModel> model)
{
    ...
    controller->setForceScaling(0.0); // Haptics off
    controller->setSmoothingKernelSize(15);
    controller->setUseForceSmoothening(true);
...
///
/// \brief Visually represents the diaphgram but also serves as a container
/// for the stomach
///
...
main()
{
...
#ifndef USE_VR
    // Start the simulation with the debug cam if not in VR
    ...
    *scene->getCamera("debug") = *scene->getActiveCamera();
    ...
    // Add the stomach
    std::shared_ptr<PbdObject> stomachObj = makeStomachObj("stomach", pbdModel);
    scene->addSceneObject(stomachObj);

    // Add the hands
    std::shared_ptr<PbdObject> leftHandObj = makeHandObj("leftHand", pbdModel);
    scene->addSceneObject(leftHandObj);
    std::shared_ptr<PbdObject> rightHandObj = makeHandObj("rightHand", pbdModel);
    ...
    // Add the esophagus capsules located inside the esophagus
    std::vector<std::shared_ptr<PbdObject>> esophagusCapsuleObjs;
    makeCapsules("capsuleObj", esophagusCapsuleObjs, pbdModel, 0.008);
    ...
    // Add diaphgram which also serves as the container for the stomach
    ...
    // Due to remeshing these won't work until meshes updated
    ...
    // Left grasping
    auto leftToolGrasping0 = std::make_shared<PbdObjectGrasping>(leftToolObj, leftHandObj);
    leftToolGrasping0->setCompliance(0.00001);
    scene->addInteraction(leftToolGrasping0);
    auto leftToolGrasping1 = std::make_shared<PbdObjectGrasping>(rightToolObj, leftHandObj);
    leftToolGrasping1->setCompliance(0.00001);
    scene->addInteraction(leftToolGrasping1);
    auto leftToolGrasping2 = std::make_shared<PbdObjectGrasping>(cameraToolObj, leftHandObj);
    leftToolGrasping2->setCompliance(0.00001);
    scene->addInteraction(leftToolGrasping2);
    auto leftStomachGrasping = std::make_shared<PbdObjectGrasping>(stomachObj);
    leftStomachGrasping->setGeometryToPick(stomachObj->getCollidingGeometry(),
        std::dynamic_pointer_cast<PointwiseMap>(stomachObj->getPhysicsToCollidingMap()));
    leftStomachGrasping->setStiffness(graspStiffness);
    scene->addInteraction(leftStomachGrasping);

    // Right grasping
    auto rightToolGrasping0 = std::make_shared<PbdObjectGrasping>(leftToolObj, rightHandObj);
    rightToolGrasping0->setCompliance(0.00001);
    scene->addInteraction(rightToolGrasping0);
    auto rightToolGrasping1 = std::make_shared<PbdObjectGrasping>(rightToolObj, rightHandObj);
    rightToolGrasping1->setCompliance(0.00001);
    scene->addInteraction(rightToolGrasping1);
    auto rightToolGrasping2 = std::make_shared<PbdObjectGrasping>(cameraToolObj, rightHandObj);
    rightToolGrasping2->setCompliance(0.00001);
    scene->addInteraction(rightToolGrasping2);
    auto rightStomachGrasping = std::make_shared<PbdObjectGrasping>(stomachObj);
    rightStomachGrasping->setGeometryToPick(stomachObj->getCollidingGeometry(),
        std::dynamic_pointer_cast<PointwiseMap>(stomachObj->getPhysicsToCollidingMap()));
    rightStomachGrasping->setStiffness(graspStiffness);
    scene->addInteraction(rightStomachGrasping);
    ...
    // Run the simulation
    {
...
#ifndef USE_VR
        // Setup a HapticManager for haptic devices
        ...
        viewer->setVtkLoggerMode(VTKOpenVRViewer::VTKLoggerMode::MUTE);  
#endif
        ...
        std::shared_ptr<DeviceClient> leftDeviceClient = nullptr;
        std::shared_ptr<DeviceClient> rightDeviceClient = nullptr;
#ifdef USE_VR
        // Setup left VR controller
        leftDeviceClient = viewer->getVRDeviceClient(OPENVR_LEFT_CONTROLLER);
        auto                                leftController   = leftHandObj->getComponent<PbdObjectController>();
        leftController->setDevice(leftDeviceClient);

        // Setup right VR controller
        rightDeviceClient = viewer->getVRDeviceClient(OPENVR_RIGHT_CONTROLLER);
        ...
        leftDeviceClient = hapticManager->makeDeviceClient("LeftDevice");
        auto                          leftController = leftHandObj->getComponent<PbdObjectController>();
        leftController->setDevice(leftDeviceClient);
...
#ifndef USE_ONE_HAPTIC_DEVICE
        rightDeviceClient = hapticManager->makeDeviceClient("RightDevice");
        auto rightController = rightHandObj->getComponent<PbdObjectController>();
        rightController->setDevice(rightDeviceClient);
        ...
        // Defines left control scheme (ie: maps buttons -> grasping)
        auto leftButtonControl = leftHandObj->addComponent<VRHandControl>();
        leftButtonControl->setDevice(leftDeviceClient);
        leftButtonControl->addToolGrasper(leftToolGrasping0);
        leftButtonControl->addToolGrasper(leftToolGrasping1);
        leftButtonControl->addToolGrasper(leftToolGrasping2);
        leftButtonControl->addOrganGrasper(leftStomachGrasping);
        leftButtonControl->setHandObject(leftHandObj);

        // Defines right control scheme (ie: maps buttons -> grasping)
        auto rightButtonControl = rightHandObj->addComponent<VRHandControl>();
        rightButtonControl->setDevice(rightDeviceClient);
        rightButtonControl->addToolGrasper(rightToolGrasping0);
        rightButtonControl->addToolGrasper(rightToolGrasping1);
        rightButtonControl->addToolGrasper(rightToolGrasping2);
        rightButtonControl->addOrganGrasper(rightStomachGrasping);
        rightButtonControl->setHandObject(rightHandObj);

        
#ifdef USE_VR
        // Add joystick cam control
        ...
        camControl->setTranslateSpeedScale(0.25);
        camControl->setVerticalSpeedScale(0.25);
        camControl->setRotateSpeedScale(1.0);
        camControl->setCamera(scene->getActiveCamera());

        // Set the buttons for the controllers
        leftButtonControl->setGraspButtons(6, 4);
        rightButtonControl->setGraspButtons(7, 5);
#else
        // Start simulation paused
        sceneManager->pause();

        // Move the camera with the cam tool
        ...
        auto txtVisualModel = controls->addComponent<TextVisualModel>();
        txtVisualModel->setPosition(TextVisualModel::DisplayPosition::UpperLeft);
        txtVisualModel->setText(
            "Haptic Button 0: Grasp tools with hands\n"
            "Haptic Button 1: Grasp tissue with held tool\n"
            "Key 'd': Switch camera views");

        // Set the buttons for the device
        leftButtonControl->setGraspButtons(0, 1);
        rightButtonControl->setGraspButtons(0, 1);
#endif
